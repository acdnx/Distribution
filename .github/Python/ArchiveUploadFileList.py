...
一、工具定位与能力
----------------------------------------------------------------------------------------
采集端每天全量扫描后产出的清单会聚合进 Branch/{branch}/UploadFileList.txt（待上传队列）；
...
本工具定期把【已确认上传成功】的行移出队列，并归档留痕：
    ...
    2. 逐个 Branch/{branch}/UploadFileList.txt：把确认已上传的行拆出；
    3. 拆出的行原样归档到 Archive/{branch}/ArchiveUploadFileList_yyyyMMddHHmmss.txt
       （同一目录段的归档，时间戳为东八区）；
    4. 队列重写为剩余行；无确认项时整步跳过（幂等，不产生提交）。

二、匹配规则（三条全满足才算“已确认上传”）
----------------------------------------------------------------------------------------
    1. 回调记录的 rel_path 与队列行第 4 段（相对路径）相同；
    2. 回调记录的 etag 与队列行第 3 段（MD5）相同（忽略大小写）；
       队列行为历史格式（仅路径、无 MD5）时，退化为只按路径匹配；
    3. **回调时间 >= 该行的最后修改时间**。
    ...
    回调文件名自带东八区时间戳（yyyyMMdd + HHmmssSSS，共 17 位），与队列行的时间戳
    同格式，可直接按字符串比较。
...
QUEUE_NAME = 'UploadFileList.txt'     # 待上传队列文件名
...
def split_row(raw):
    ...
    :param raw: 原始行文本
    :return: (行时间戳, 行 MD5 小写, 相对路径, 原始行)
             历史格式（仅路径）时前两者为 None
    """
    fields = raw.split('|', 3)      # 至多切 3 次 → 第 4 段保留其余全部内容
    if len(fields) == 4:
        ts, _sha1, md5, rel_path = fields
        return (ts.strip() or None, md5.strip().lower() or None,
                rel_path.strip(), raw)
    return None, None, raw.strip(), raw
...
def is_confirmed(rel_path, row_md5, row_ts, index):
    ...
    :param row_md5: 行的 MD5（小写）；历史格式行为 None
    :param row_ts: 行的最后修改时间戳；历史格式行为 None
...
def archive_dir(branch, index, ts):
    ...
    archived, kept = [], []
    for raw in rows:
        row_ts, row_md5, rel_path, _ = split_row(raw)
    ...
    archive_path = os.path.join(
        archive_dir_path, '%s_%s.txt' % (ARCHIVE_PREFIX, ts))
