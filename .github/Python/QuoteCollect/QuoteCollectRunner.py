...
#: 注意：行情库（含 Ed25519 / RSA-SHA256 签名及纯标准库密码学实现）与**采集侧取数逻辑**
#: 已内联到本文件；MVSV 生成逻辑在同目录公共模块 `MvsvWriter`。
#: 因此本文件的仓库内依赖有两项：
#:     1. `GitHubCommitContent`（Contents API 封装，上一级目录）—— 既有公共模块，刻意不内联；
#:     2. `MvsvWriter`（同目录）—— MVSV 格式、数据模型与序列化，供本脚本与后续模块复用。
#: 故本文件不是"拷到哪都能跑"的完全自包含形态：**需与这两个模块同仓部署**
